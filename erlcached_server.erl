...
%% This module  is the backend of  erlcached, i.e. the cache  itself. It
%% implements  the operations  that can  be performed  on the  cache and
%% keeps track of  various cache statistics. The cache itself  is an ETS
%% table. There is a second table  that is used for storing metadata and
...
%% metadata_table stores the metadata about  the cache table and various
%% runtime statistics and state.
